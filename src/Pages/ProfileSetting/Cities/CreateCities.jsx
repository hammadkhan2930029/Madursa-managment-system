...
//     // Outside click par dropdown band karne ke liye
//     useEffect(() => {
...
//     // Filter Logic: Agar search empty hai to sab dikhaye, warna match dikhaye
//     const filteredCities = allCities.filter(city => {
...
//                 <div className="text-right">
//                     <h2 className="text-2xl font-black text-slate-800 tracking-tight">شہروں کی فہرست</h2>
//                     <p className="text-sm text-slate-500 font-medium">کل شہر: {addedCities.length}</p>
//                 </div>
//                 <button
//                     onClick={() => setIsFormOpen(!isFormOpen)}
//                     className={`flex items-center gap-3 px-8 py-4 rounded-2xl font-black text-sm transition-all shadow-lg ${isFormOpen ? 'bg-rose-50 text-rose-500' : 'bg-[#00d094] text-white'}`}
//                 >
//                     {isFormOpen ? 'بند کریں' : 'نیا شہر شامل کریں'}
//                     {isFormOpen ? <X size={20} /> : <Plus size={20} />}
//                 </button>
...
//             {/* --- SEARCHABLE SELECT FORM --- */}
//             {isFormOpen && (
//                 <div className="bg-white/80 backdrop-blur-xl border border-[#00d094]/30 shadow-2xl rounded-[2.5rem] p-8">
//                     <div className="max-w-md mx-auto space-y-4">
//                         <label className="text-[11px] font-black text-slate-500 mr-2 block text-right tracking-widest">شہر منتخب کریں *</label>

//                         <div className="relative" ref={dropdownRef}>
//                             {/* Input Field as Trigger */}
//                             <div className="relative group">
//                                 <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-[#00d094]">
//                                     <Search size={18} />
//                                 </div>
//                                 <input
//                                     type="text"
//                                     placeholder="شہر تلاش کریں یا لسٹ سے چنیں..."
//                                     value={searchTerm}
...
//                                     }}
//                                     className="w-full bg-white border-2 border-slate-100 focus:border-[#00d094] outline-none h-[64px] pr-12 pl-12 rounded-2xl text-lg font-bold text-right transition-all shadow-sm"
//                                 />
//                                 <div 
//                                     className="absolute inset-y-0 left-4 flex items-center cursor-pointer text-slate-400"
...
//                             {/* --- SCROLLABLE LIST --- */}
//                             {isOpen && (
//                                 <div className="absolute z-50 w-full mt-2 bg-white border border-slate-100 shadow-2xl rounded-2xl max-h-[280px] overflow-y-auto overflow-x-hidden p-2 border-t-4 border-t-[#00d094] animate-in zoom-in-95 duration-200 vip-scrollbar">
//                                     {filteredCities.length > 0 ? (
//                                         filteredCities.map((city, index) => (
//                                             <div
//                                                 key={index}
...
//                                                     setIsOpen(false);
//                                                 }}
//                                                 className={`p-4 mb-1 rounded-xl cursor-pointer font-bold text-right transition-all flex items-center justify-between hover:bg-emerald-50 group ${
//                                                     selectedCity === city ? 'bg-emerald-100 text-[#00d094]' : 'text-slate-700 hover:text-[#00d094]'
//                                                 }`}
//                                             >
//                                                 <div className="flex items-center gap-3">
//                                                     {selectedCity === city ? <Check size={18} /> : <MapPin size={16} className="text-slate-300 group-hover:text-[#00d094]" />}
//                                                     <span>{city}</span>
//                                                 </div>
//                                             </div>
//                                         ))
//                                     ) : (
//                                         <div className="p-8 text-center text-slate-400 font-bold">کوئی شہر نہیں ملا</div>
//                                     )}
//                                 </div>
//                             )}
//                         </div>
...
//                             onClick={handleAddCity}
//                             disabled={!selectedCity}
//                             className={`w-full mt-4 py-4 rounded-xl font-black text-sm shadow-xl transition-all flex items-center justify-center gap-3 ${
//                                 selectedCity ? 'bg-[#218838] text-white hover:scale-[1.02]' : 'bg-slate-100 text-slate-300 cursor-not-allowed'
//                             }`}
//                         >
//                             فہرست میں شامل کریں <Plus size={20} />
//                         </button>
...
//                                 <th className="px-6 py-4">ایکشن</th>
//                             </tr>
//                         </thead>
//                         <tbody>
//                             {addedCities.map((city, index) => (
//                                 <tr key={city.id} className="bg-white/60 hover:bg-white transition-all duration-300 group rounded-2xl shadow-sm">
//                                     <td className="px-6 py-4 font-bold text-slate-500 first:rounded-r-2xl">{addedCities.length - index}</td>
...
//                                         </div>
//                                     </td>
//                                     <td className="px-6 py-4 last:rounded-l-2xl">
//                                         <button 
//                                             onClick={() => setAddedCities(addedCities.filter(c => c.id !== city.id))}
//                                             className="p-2.5 bg-rose-50 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all shadow-sm"
//                                         >
